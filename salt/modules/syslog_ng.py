...
def __virtual__():
    '''
    Initializes the module and returns "syslog_ng", if it is available
    '''
...
def ctl(command="stats"):
    ...
    if command not in commands:
        return ("The given command '{0}' is not among the supported ones. "
                "Please run syslog-ng-ctl for help.")
...
def start(*args):
    """
    Starts syslog-ng with the supplied arguments

    salt '*' syslog_ng.start -Fevd
    """
    ret = _run_command("syslog-ng", options=args)
    if ret["retcode"] == 0:
        return 0
    else:
        return ret["stderr"]
